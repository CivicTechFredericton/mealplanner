...
DO
LANGUAGE plpgsql
$$ DECLARE admn app.person;
user1 app.person;
user2 app.person;
meal_designer app.person;
person_pass text;
BEGIN
  CREATE OR REPLACE FUNCTION app_private.product_id_by_upc(c text)
    RETURNS bigint
    AS $f$
  SELECT
    id
...
LANGUAGE sql
STABLE;
CREATE OR REPLACE FUNCTION app_private.meal_id_by_code(c text)
  RETURNS bigint
  AS $f$
  SELECT
    id
...
VALUES
  (
    ...
    NULL,
    NULL,
    '1. Clean the bunch of coriander leaves thoroughly.  2. grind 1 cup of coconut chunks to powder in a mixie 3. Then add 3/4th cup of roasted gram,  3 red chilli, 1 inch ginger and grind it 4. Then add coriander leaves, salt to taste and 1/2 cup of water and grind it to a paste.  5. Serve it is a thottukai for idli, dosa, vada, bonda or bajji',
    '11.15',
    '2',
  ...
  (
    ...
    NULL,
    NULL,
    '1. Wash and dry the cucumber. The skin can be left on or partially removed. If using field cucumbers, peel and remove seeds prior to slicing.  2. Slice the cucumbers as thinly as possible (almost see through). Set aside 3. Place the vinegar, sugar, salt and pepper in a mixing bowl and whisk until the sugar has dissolved completely 4. Add the cucumber, onion and dill. Mix together well 5. Refrigerate, turning occasionally, for at least 1 hour',
    0,
    0,
    'Defreeze the coconut chunks before 30 minutes to obtain soft texture. You can grind the coconut chunks to a poweder and store it in an airtight container.',
    0,
  ...
  (
    ...
    NULL,
    NULL,
    'Preheat oven to 325 f  1. Bring 3 litres of water, with 1 Tsp. (5 ml) of salt to the boil. Add the macaroni noodles and cook following package directions. Stir occasionally during cooking. Drain and rinse well under cold water. Place in a colander and set aside.  2. For the sauce - In a large saucepan over medium heat, melt the butter. Add the flour and mix well. Lower heat and cook for about 5 minutes. Stir frequently.  3. Heat the milk in a separate pan until warm and slowly add to the butter/flour mixture (the roux) over low heat, stirring frequently, cook until sauce thickens slightly, about 10-15 minutes.   4. Turn the heat to very low, cover the saucepan and let sit for about 20 minutes, stirring occasionally, to cook out the taste of the flour. Add the grated cheese, salt and pepper. Stir until the cheese has melted.  5. Add the noodles to the sauce and mix well. Place in a greased 8” x 8” deep baking dish. If the sauce appears too thick, add some warmed milk until desired consistency is reached. Remember, sauce will thicken during cooking as the pasta will absorb the sauce.  6. Sprinkle the breadcrumbs over the macaroni and dot with butter or margarine. For a spicier topping sprinkle with the chili powder. Bake uncovered in pre-heated oven for about 30-35 minutes or until the sauce has bubbled around the edges and the top is nicely browned. Allow to sit for a few minutes before serving.',
    0,
    0,
    '',
    0,
...
VALUES
  (
    ...
    1.6,
    8.2,
    app_private.meal_id_by_code ('cc_side'),
    'meal'
  );
...
VALUES
  (
    ...
    0,
    3,
    app_private.product_id_by_upc ('80914514137'),
    'product'
  );
SELECT
  substring(encode(digest(random()::text, 'sha256'), 'hex')
  FROM 0 FOR 16) INTO person_pass;
...
PERFORM
  app.authorize_meal_designer(meal_designer.id);
...
VALUES
  (
    ...
    'This meal plan will cater to egg-free vegetarian meals with diary products',
    '{"vegetarian","egg-free"}'
  );
END;
$$;
